<template>
    <div class="list-view">  
      <div v-for="(dayGroup, index) in groupedByDay" :key="index" class="mb-4 border rounded shadow-sm"> 
        <div class="flex flex-row w-full bg-blue-400" > 
            <div> {{ dayGroup.titleRow.date }}: </div> 
            <div> 
              <span v-if="dayGroup.titleRow.titleLabel"> {{ dayGroup.titleRow.titleLabel }}</span>  

              <div v-for="(noteItem, noteIndex) in dayGroup.titleRow.notes" :key="noteIndex" class=" "> 
                <span v-if="noteItem.cellType === 'link'">
                  <a
        ...
        <div class="flex flex-col" v-for="(item, itemIndex) in dayGroup.rowsUnderGroup">
          ...
          <div class="flex flex-row w-full bg-amber-100"> 
            <div>{{ item.date }}</div>
            <div>
              <div class="bg-red-300">{{ item.activity }}  </div>
