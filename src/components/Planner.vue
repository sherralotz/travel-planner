      ...
      <div v-if="currentStep === 1" class="max-w-2xl mx-auto">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div 
            ...
            :class="{'border-blue-500 bg-blue-50': selectedDestination && selectedDestination.id === destination.id}"
            @click="selectDestination(destination)"
          >
            <h3 class="font-bold">{{ destination.name }}</h3>
            <p class="text-sm text-gray-600">{{ destination.country }}</p>
            <p class="mt-2 text-sm">{{ destination.description }}</p>
          </div>
        </div>
      ...
      <div v-if="currentStep === 3" class="max-w-2xl mx-auto">
        ...
        <div class="space-y-4">
          <div 
            ...
            :key="preference.id"
            class="border rounded-lg p-4 cursor-pointer hover:bg-blue-50 transition"
            :class="{'border-blue-500 bg-blue-50': selectedPreferences.includes(preference.id)}"
            @click="togglePreference(preference.id)"
          >
            <h3 class="font-bold">{{ preference.name }}</h3>
      ...
      <div v-if="currentStep === 4" class="max-w-4xl mx-auto">
        ...
        <div v-else-if="itinerary" class="space-y-8">
          ...
          <div v-if="activeTab === 'daily'" class="space-y-6">
            <div v-for="(day, index) in itinerary.dailyPlan" :key="index" class="border rounded-lg p-4">
              <h3 class="font-bold text-lg border-b pb-2 mb-3">Day {{ index + 1 }}</h3>
              <div class="space-y-4">
                <div v-for="(activity, actIndex) in day" :key="actIndex" class="flex">
                  <div class="font-medium w-20 text-gray-600">{{ activity.time }}</div>
                  <div>
          ...
          <div v-if="activeTab === 'food'" class="space-y-4">
            <div v-for="(spot, index) in itinerary.foodSpots" :key="index" class="border rounded-lg p-4">
              <h3 class="font-bold">{{ spot.name }}</h3>
              <p class="text-sm text-gray-600">{{ spot.type }} • {{ spot.priceRange }}</p>
              <p class="mt-2">{{ spot.description }}</p>
              <p class="mt-1 text-sm">Address: {{ spot.address }}</p>
            </div>
          </div>
  
          <!-- Activities Tab -->
          <div v-if="activeTab === 'activities'" class="space-y-4">
            <div v-for="(activity, index) in itinerary.activities" :key="index" class="border rounded-lg p-4">
              <h3 class="font-bold">{{ activity.name }}</h3>
              <p class="text-sm text-gray-600">{{ activity.category }} • {{ activity.duration }}</p>
          ...
          <div v-if="activeTab === 'tips'" class="space-y-4">
            <div v-for="(category, index) in itinerary.travelTips" :key="index" class="border rounded-lg p-4">
              <h3 class="font-bold">{{ category.category }}</h3>
              <ul class="mt-2 space-y-2">
                <li v-for="(tip, tipIndex) in category.tips" :key="tipIndex" class="ml-4 list-disc">
                  {{ tip }}
                </li>
              </ul>
            </div>
          ...
          <div v-if="activeTab === 'packing'" class="space-y-4">
            <div v-for="(category, index) in itinerary.packingList" :key="index" class="border rounded-lg p-4">
              <h3 class="font-bold">{{ category.category }}</h3>
              <div class="mt-2 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                <div v-for="(item, itemIndex) in category.items" :key="itemIndex" class="flex items-center">
                  <input type="checkbox" :id="'item-' + index + '-' + itemIndex" class="mr-2">
