      ...
      <div class="relative bg-white rounded-lg shadow-xl w-full max-w-md">
        ...
        <div class="p-6 space-y-4">
          <div>
            ...
            <input type="text" id="label" v-model="linkData.label" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:outline-none block w-full p-2.5" placeholder="Link Text" required>
          </div>
          <div>
            <label for="url" class="block mb-2 text-sm font-medium text-gray-900">URL</label>
            <input type="url" id="url" v-model="linkData.url" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:outline-none block w-full p-2.5" placeholder="https://example.com" required>
          </div>
        </div>
        <div class="flex items-center p-4 space-x-2 border-t border-gray-200 rounded-b">
          <button @click="saveLink" type="button" class="text-white bg-blue-700 hover:bg-blue-800 bg-red-500 hover:bg-red-400 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Save Link</button>
